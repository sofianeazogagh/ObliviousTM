...
pub fn demultiplixer()
{
    ...
    let sks : ServerKey = ServerKey { 
        key_switching_key: public_key.lwe_ksk.to_owned(),
        bootstrapping_key: public_key.fourier_bsk.to_owned(),
    ...
    let mut not_ct_cp = LweCiphertext::new(0_64,ctx.big_lwe_dimension().to_lwe_size(), ctx.ciphertext_modulus());
    ...
    let mut not_ct_cp = LweCiphertext::new(0,ctx.small_lwe_dimension().to_lwe_size(), ctx.ciphertext_modulus());
    lwe_ciphertext_sub(&mut not_ct_cp, &ct_one, &ct_cp);
...
-> LweCiphertext<Vec<u64>>
{
    ...
    let ct1  = Ciphertext { ct: lwe_ciphertext_1.to_owned(),
                                        degree: tfhe::shortint::ciphertext::Degree(message_modulus.0 - 1), 
                                        message_modulus, 
                                        carry_modulus,
                                    };
    
    let ct2  = Ciphertext { ct: lwe_ciphertext_2.to_owned(),
                                            degree: tfhe::shortint::ciphertext::Degree(message_modulus.0 - 1), 
                                            message_modulus, 
                                            carry_modulus
                                        };
